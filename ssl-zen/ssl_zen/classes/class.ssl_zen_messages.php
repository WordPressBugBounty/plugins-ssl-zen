    ...
    class ssl_zen_messages
    {
        ...
        public static function getMessages()
        {
            ...
            return array(
                ...
                'token_missmatch' => [
                    'msg' => __(
                        'Token does not match the content of ' . base64_decode(sanitize_text_field($_REQUEST['uri'])) . (isset($_REQUEST['host']) ? '. Either ' . base64_decode(sanitize_text_field($_REQUEST['host'])) . ' is not pointed
...
The Duplicate Certificate limit (5 certificates with the exact same set of domains per week) has been exceeded and is affecting the domain "%1$s". The exact set of domains affected is: "%1$s". It may be possible to avoid this rate limit by issuing a certificate with an additional or different domain name.', 'ssl-zen'),
                ...
                'lets_encrypt_error_issuefromletsencrypt' => [
                    'msg' => __(
                        'A test authorization for your domain to the Let\'s Encrypt staging service has revealed issues that may prevent any certificate for this domain being issued.
...
A timeout was experienced while communicating with %1$s/1.2.3.4: Get http://%1$s/.well-known/acme-challenge/letsdebug-test: dial tcp 1.2.3.4:80: i/o timeout', 'ssl-zen'),
                ...
                'invalid_ip_address' => [
                    'msg' => __('Due to some network restrictions we\'re unable to validate your IP address. Please send an email to support@sslzen.com', 'ssl-zen'),
                    'type' => 'error'
                ]
            );
        }
